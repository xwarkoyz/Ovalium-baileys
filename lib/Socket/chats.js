            ...
            const verifiedResults = await Promise.all(
                ...
                .map(async ({ contact, id, lid }) => {
                    try {
                        ...
                        const profileNode = (0, WABinary_1.getBinaryNodeChild)(result, 'business_profile');
                        const isBusiness = !!(0, WABinary_1.getBinaryNodeChild)(profileNode, 'profile');
                        return {
                            jid: id,
                            exists: true,
                            lid: lid,
                            status: isBusiness ? 'business' : 'regular'
                        };
                    } catch (error) {
                        return {
                            jid: id,
